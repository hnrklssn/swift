...
extension SpanBoundsThunkBuilder {
  ...
  var genericArg: TypeSyntax {
    get throws {
      ...
      if genericArgumentClause.arguments.count != 1 {
        throw DiagnosticError(
          "expected exactly 1 generic type argument for std::span type '\(idType)', found \(genericArgumentClause.arguments.count)",
          node: genericArgumentClause.arguments)
      }
      guard let arg = TypeSyntax(genericArgumentClause.arguments.first!.argument) else {
        throw DiagnosticError(
          "invalid generic type argument '\(genericArgumentClause.arguments.first!.argument)'",
          node: genericArgumentClause.arguments.first!.argument)
      }
      return arg
    }
  }
